...
(CONDIZIONI, SCEGLI_LINGUA, GDPR, NOME, INDIRIZZO, TELEFONO,
 FOTO_TARGHETTA, MARCA, MODELLO, SERIALE,
 PROBLEMA, FOTO_MACCHINA, CONFERMA) = range(13)
...
TESTI = {
    "it": {
            ...
            "━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "Ai sensi degli artt. 13-14 del Reg. UE 2016/679 (GDPR), i Suoi dati personali "
            "(nome, indirizzo, telefono, foto) saranno trattati da:\n\n"
            "🏢 *Rotondi Group Srl*\n"
            "📍 Via F.lli Rosselli 14/16\n"
    ...
    "en": {
            ...
            "🔒 *PRIVACY NOTICE (GDPR)*\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "Pursuant to Arts. 13-14 EU Reg. 2016/679 (GDPR), your personal data "
            "(name, address, phone, photos) will be processed by:\n\n"
            "🏢 *Rotondi Group Srl*\n"
            "📍 Via F.lli Rosselli 14/16\n"
            "20019 Settimo Milanese (MI)\n"
            "📧 segnalazioni-privacy@rotondigroup.it\n\n"
            "• *Purpose:* technical assistance request management\n"
            "• *Retention:* max 2 years\n"
        ...
        "gdpr_denied": (
            ...
            "Without consent we cannot proceed.\n"
    ...
    "bn": {
                "gdpr": (
            "🔒 *গোপনীয়তা নোটিশ (GDPR)*\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "আপনার ব্যক্তিগত তথ্য প্রক্রিয়া করবে:\n\n"
            "🏢 *Rotondi Group Srl*\n"
            "📍 Via F.lli Rosselli 14/16\n"
            "20019 Settimo Milanese (MI)\n"
            "📧 segnalazioni-privacy@rotondigroup.it\n\n"
            "• *উদ্দেশ্য:* প্রযুক্তিগত সহায়তা পরিচালনা\n"
            "• *সংরক্ষণ:* সর্বোচ্চ ২ বছর\n\n"
            "আপনি কি আপনার ব্যক্তিগত তথ্য প্রক্রিয়াকরণে সম্মতি দেন?"
        ),
        "gdpr_si": "✅ সম্মতি দিচ্ছি",
        "gdpr_no": "❌ সম্মতি দিচ্ছি না",
        "gdpr_denied": (
            "❌ *সম্মতি প্রদান করা হয়নি*\n\n"
            "সম্মতি ছাড়া আমরা এগিয়ে যেতে পারছি না।\n"
    ...
    "ar": {
            ...
            "🔒 *إشعار الخصوصية (GDPR)*\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "ستُعالج بياناتك الشخصية من قِبَل:\n\n"
            "🏢 *Rotondi Group Srl*\n"
            "📍 Via F.lli Rosselli 14/16\n"
            "20019 Settimo Milanese (MI)\n"
            "📧 segnalazioni-privacy@rotondigroup.it\n\n"
            "• *الغرض:* إدارة طلب المساعدة التقنية\n"
            "• *الاحتفاظ:* سنتان كحد أقصى\n\n"
            "هل توافق على معالجة بياناتك الشخصية؟"
        ),
        "gdpr_si": "✅ أوافق",
        "gdpr_no": "❌ لا أوافق",
        "gdpr_denied": (
            "❌ *لم تُقدَّم الموافقة*\n\n"
            "بدون موافقة لا يمكننا المتابعة.\n"
...
async def gestisci_gdpr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "gdpr_no":
        await query.edit_message_text(
            t(lingua, "gdpr_denied"),
            parse_mode="Markdown"
        )
        return ConversationHandler.END
    # Accettato — mostra condizioni
    kb = InlineKeyboardMarkup([[
...
def calcola_preventivo_bot(indirizzo_cliente):
    """Calcola preventivo usando Google Maps Distance Matrix API"""
    try:
        ...
        if dist_km < 10:
            ...
            dist_ar  = dist_km * 2
            dur_ar   = math.ceil(dur_h * 2)  # arrotonda ore viaggio A/R all'ora superiore
...
async def raccogli_indirizzo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Calcola preventivo — SOLO per fuori Roma
    prev = calcola_preventivo_bot(indirizzo)
    if prev and prev["zona"] == "outside_gra":
        context.user_data["preventivo"] = prev
        PREV_TESTI = {
            "it": (
                f"\U0001f4b0 *PREVENTIVO INDICATIVO*\n{'─'*24}\n"
                f"📍 *Zona:* {'Zona di Roma (dentro GRA)' if prev['zona']=='inside_gra' else 'Provincia di Roma, Lazio e resto d Italia'}\n"
                f"🗺 *Distanza:* {prev['dest_label']} — {prev['dur_label']}\n\n"
                f"{prev['dettaglio_it']}\n\n"
                f"💶 *Costo minimo stimato:* €{prev['costo_min']:.2f} + IVA\n"
                f"{'─'*24}\n"
                f"_Preventivo indicativo per 1h di lavoro. Il costo finale dipende dall'effettivo tempo di intervento._"
            ),
            "en": (
                f"\U0001f4b0 *INDICATIVE QUOTE*\n{'─'*24}\n"
                f"📍 *Zone:* {'Rome area (inside GRA)' if prev['zona']=='inside_gra' else 'Rome province, Lazio and rest of Italy'}\n"
                f"🗺 *Distance:* {prev['dest_label']} — {prev['dur_label']}\n\n"
                f"{prev['dettaglio_it']}\n\n"
                f"💶 *Estimated minimum cost:* €{prev['costo_min']:.2f} + VAT\n"
                f"{'─'*24}\n"
                f"_Indicative quote for 1h work. Final cost depends on actual intervention time._"
            ),
            "bn": (
                f"\U0001f4b0 *আনুমানিক খরচ*\n{'─'*24}\n"
                f"📍 *দূরত্ব:* {prev['dest_label']} — {prev['dur_label']}\n\n"
                f"{prev['dettaglio_it']}\n\n"
                f"💶 *আনুমানিক সর্বনিম্ন খরচ:* €{prev['costo_min']:.2f} + IVA"
            ),
            "zh": (
                f"\U0001f4b0 *预估报价*\n{'─'*24}\n"
                f"📍 *区域:* {'罗马市区' if prev['zona']=='inside_gra' else '罗马省及意大利其他地区'}\n"
                f"🗺 *距离:* {prev['dest_label']} — {prev['dur_label']}\n\n"
                f"{prev['dettaglio_it']}\n\n"
                f"💶 *预估最低费用:* €{prev['costo_min']:.2f} + 增值税"
            ),
            "ar": (
                f"\U0001f4b0 *عرض سعر تقريبي*\n{'─'*24}\n"
                f"📍 *المنطقة:* {'منطقة روما' if prev['zona']=='inside_gra' else 'محافظة روما وبقية ايطاليا'}\n"
                f"🗺 *المسافة:* {prev['dest_label']} — {prev['dur_label']}\n\n"
                f"{prev['dettaglio_it']}\n\n"
                f"💶 *الحد الادنى للتكلفة:* €{prev['costo_min']:.2f} + ضريبة"
            ),
        }
        testo_prev = PREV_TESTI.get(lingua, PREV_TESTI["it"])
        await update.message.reply_text(testo_prev, parse_mode="Markdown")
...
async def conferma(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Aggiungi preventivo alla notifica tecnici
    prev = context.user_data.get("preventivo")
    prev_text = ""
    if prev:
        prev_text = (
            f"\n💰 *Preventivo:* €{prev['costo_min']:.2f} + IVA"
            f" ({prev['dest_label']} — {prev['dur_label']})"
        )
...
def main():
    ...
    t.start()
    log.info(f"🌐 Health server avviato")
